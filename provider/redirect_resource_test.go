...
func TestRedirectCreate_ValidationErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"invalid siteId", RedirectArgs{SiteID: "invalid", SourcePath: "/old", DestinationPath: "/new", StatusCode: 301}, "validation failed"},
		{"missing sourcePath", RedirectArgs{SiteID: testSiteID, SourcePath: "", DestinationPath: "/new", StatusCode: 301}, "sourcePath is required"},
		{"sourcePath without slash", RedirectArgs{SiteID: testSiteID, SourcePath: "old-page", DestinationPath: "/new", StatusCode: 301}, "must start with '/'"},
		{"missing destinationPath", RedirectArgs{SiteID: testSiteID, SourcePath: "/old", DestinationPath: "", StatusCode: 301}, "destinationPath is required"},
		{"destinationPath without slash", RedirectArgs{SiteID: testSiteID, SourcePath: "/old", DestinationPath: "new", StatusCode: 301}, "must start with '/'"},
		{"invalid statusCode", RedirectArgs{SiteID: testSiteID, SourcePath: "/old", DestinationPath: "/new", StatusCode: 400}, "must be either 301 or 302"},
		{"sourcePath with query string", RedirectArgs{SiteID: testSiteID, SourcePath: "/page?query=value", DestinationPath: "/new", StatusCode: 301}, "invalid characters"},
		{"destinationPath with hash", RedirectArgs{SiteID: testSiteID, SourcePath: "/old", DestinationPath: "/page#anchor", StatusCode: 301}, "invalid characters"},
	}
...
func TestRedirectCreate_UsesCreatedOnFromAPI(t *testing.T) {
	mockWebflowAPI(t, func(w http.ResponseWriter, r *http.Request) {
		writeJSON(t, w, http.StatusOK, `{"id":"`+testRedirectID+`","fromUrl":"/a","toUrl":"/b","createdOn":"2024-01-15T10:30:00Z"}`)
	})
	resp, err := (&Redirect{}).Create(context.Background(), infer.CreateRequest[RedirectArgs]{
...
func TestRedirectRead_FindsRedirectOnLaterPage(t *testing.T) {
	...
	mockWebflowAPI(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case "0", "":
			...
			writeJSON(t, w, http.StatusOK, `{"redirects":[{"id":"other-1","fromUrl":"/x","toUrl":"/y"}],"pagination":{"limit":1,"offset":0,"total":3}}`)
		case "1":
			offsets = append(offsets, 1)
			writeJSON(t, w, http.StatusOK, `{"redirects":[{"id":"other-2","fromUrl":"/x2","toUrl":"/y2"}],"pagination":{"limit":1,"offset":1,"total":3}}`)
		case "2":
			offsets = append(offsets, 2)
			writeJSON(t, w, http.StatusOK, `{"redirects":[{"id":"`+testRedirectID+`","fromUrl":"/old-page","toUrl":"/drifted"}],"pagination":{"limit":1,"offset":2,"total":3}}`)
		default:
	...
	resp, err := redirect.Read(context.Background(), infer.ReadRequest[RedirectArgs, RedirectState]{
		...
		Inputs: RedirectArgs{SiteID: testSiteID, SourcePath: "/old-page", DestinationPath: "/new-page", StatusCode: 301},
		State: RedirectState{
			RedirectArgs: RedirectArgs{SiteID: testSiteID, SourcePath: "/old-page", DestinationPath: "/new-page", StatusCode: 302},
	...
	if resp.Inputs.DestinationPath != "/drifted" || resp.Inputs.SourcePath != "/old-page" || resp.Inputs.SiteID != testSiteID {
...
func TestRedirectRead_MissingRedirectSignalsDeletion(t *testing.T) {
	mockWebflowAPI(t, func(w http.ResponseWriter, r *http.Request) {
		writeJSON(t, w, http.StatusOK, `{"redirects":[{"id":"other","fromUrl":"/x","toUrl":"/y"}],"pagination":{"limit":100,"offset":0,"total":1}}`)
	})
	resp, err := (&Redirect{}).Read(context.Background(), infer.ReadRequest[RedirectArgs, RedirectState]{
		ID: testRedirectResourceID(),
	})
...
func TestRedirectRead_InvalidIDs(t *testing.T) {
	called := false
	mockWebflowAPI(t, func(w http.ResponseWriter, r *http.Request) { called = true })
	for _, id := range []string{"bad", "not-a-site/redirects/" + testRedirectID, testSiteID + "/redirects/", testSiteID + "/redirects/a/b"} {
...
func TestRedirectUpdate_PatchesInPlace(t *testing.T) {
	...
	resp, err := redirect.Update(context.Background(), infer.UpdateRequest[RedirectArgs, RedirectState]{
		...
		State: RedirectState{
			RedirectArgs: RedirectArgs{SiteID: testSiteID, SourcePath: "/old-page", DestinationPath: "/new-page", StatusCode: 301},
			CreatedOn:    "2024-01-15T10:30:00Z",
		},
	})
	if err != nil {
	...
	if resp.Output.DestinationPath != "/updated-page" || resp.Output.StatusCode != 302 || resp.Output.CreatedOn != "2024-01-15T10:30:00Z" {
...
func TestRedirectUpdate_ValidationErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"invalid siteId", RedirectArgs{SiteID: "invalid", SourcePath: "/old", DestinationPath: "/new", StatusCode: 301}, "validation failed"},
		{"missing destinationPath", RedirectArgs{SiteID: testSiteID, SourcePath: "/old", DestinationPath: "", StatusCode: 301}, "destinationPath is required"},
		{"invalid statusCode", RedirectArgs{SiteID: testSiteID, SourcePath: "/old", DestinationPath: "/new", StatusCode: 303}, "must be either 301 or 302"},
		{"siteId mismatch", RedirectArgs{SiteID: testOtherSiteID, SourcePath: "/old", DestinationPath: "/new", StatusCode: 301}, "does not match"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestRedirectDelete_Success(t *testing.T) {
	...
	if _, err := (&Redirect{}).Delete(context.Background(), infer.DeleteRequest[RedirectState]{ID: testRedirectResourceID()}); err != nil {
